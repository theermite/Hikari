//! Connexion en lecture + réponse au chat Twitch (IRC), via le crate `twitch-irc` —
//! préféré à une implémentation manuelle du protocole (ce que l'outil de référence
//! `chat-overlay` fait à la main, faute de mieux en Python/Qt) : le crate porte déjà la
//! reconnexion, le PING/PONG et le parsing des tags, jamais réécrits ici.
...
/// Ce qu'une commande garde pour répondre et couper la connexion. `Clone` : le crate
/// documente le client comme une poignée clonable pour un usage multi-thread — c'est ce
/// qui permet à `chat_send` de le sortir du verrou avant un `.await` (voir `chat/mod.rs`).
#[derive(Clone)]
...
/// `oauth:` prefix"), jamais ajouté ici.
pub fn connect(
    app: AppHandle,
    login: String,
    access_token: &Secret,
...
/// Envoie `text` sur le salon déjà rejoint — la réponse (F-030 « réponse », cette partie).
