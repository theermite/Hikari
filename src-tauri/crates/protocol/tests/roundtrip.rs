...
use hikari_protocol::{
    parse_controller_command, parse_engine_message, to_line, CameraDevice, ControllerCommand,
    EngineMessage, SourceInfo, MONITOR_CAPTURE_KIND,
};
...
fn camera_device_strategy() -> impl Strategy<Value = CameraDevice> {
    (any::<String>(), any::<String>())
        .prop_map(|(name, device_id)| CameraDevice { name, device_id })
}
...
fn engine_message_strategy() -> impl Strategy<Value = EngineMessage> {
    prop_oneof![
        ...
        (
            ...
            .prop_map(|(names, active)| EngineMessage::SceneList {
                scenes: names
                    .iter()
                    .map(hikari_protocol::SceneInfo::empty)
                    .collect(),
                active,
            }),
    ]
...
fn controller_command_strategy() -> impl Strategy<Value = ControllerCommand> {
    prop_oneof![
        ...
        (any::<String>(), any::<String>())
            .prop_map(|(device_id, scene)| ControllerCommand::AddCamera { device_id, scene }),
        (any::<String>(), any::<String>(), any::<bool>()).prop_map(
            |(device_id, scene, enabled)| {
                ControllerCommand::SetBackgroundRemoval {
                    device_id,
                    scene,
                    enabled,
                }
...
fn should_list_sources_when_scene_created() {
    ...
    let msg = EngineMessage::Sources {
        items: sources.clone(),
    };
    assert_eq!(sources.len(), 1, "exactly one source in the scene");
    assert_eq!(
        sources[0].kind, MONITOR_CAPTURE_KIND,
        "source is a screen capture"
    );
    let line = to_line(&msg).expect("sources message serializes");
    assert!(
        line.contains(MONITOR_CAPTURE_KIND),
        "kind travels on the wire"
    );
