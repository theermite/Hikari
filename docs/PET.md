...
| **B-filtres** *(hors numérotation PET — né d'une clarification de Jay le 2026-09-09)* | Pile de filtres **illimitée** sur toute source (forme calculée, contour, couleur, flou...), catalogue vérifié par type de source, palette visuelle + aperçu en direct, styles sauvegardables/exportables/partageables (F-036, F-036b, détail §3quater du CDC) | Standard (même niveau que B-cam, dont elle élargit la portée) | 🟧 **sélecteur de forme caméra prouvé à l'écran 2026-09-10** (Aucun/Cercle/Coins arrondis, rayon réglable) et publié en 0.5.5 — robustesse de la relance caméra fermée dans le même lot (8 passages de relecture indépendante). Reste : contour (filtre séparé après le masque, chaînage standard OBS, confirmé par Jay — nécessite une investigation à part, `mask_color_filter.effect` remplace la couleur plutôt que dessiner un anneau), couleur/flou, catalogue par type de source, styles sauvegardables, et l'élargissement à toute source au-delà de la caméra |
...
| 3 | **La caméra qui décroche se relève** — détecter une source qui ne délivre plus, la relancer | ⛔ **testée en conditions réelles 2026-09-13 (débranchement physique), puis COUPÉE le même soir** : le premier essai a d'abord révélé que `sample_frame` ne comptait jamais un vrai débranchement comme un gel (corrigé), puis, une fois corrigé, le contrôle automatique a relancé une caméra SAINE en boucle — `obs_source_get_frame` appelé toutes les 3 s depuis notre fil perturbe probablement le pipeline async de la source, sans confirmation encore. Décision de Jay : on s'arrête là pour la caméra, priorité à d'autres briques. `check_camera_health`/`frame_timestamp` restent dans le code (morts, non appelés) pour une reprise future avec un signal de gel prouvé sans effet de bord. **Reste acquis et livré** : bouton « Relancer » (réglages) + icône ↻ directement sur la ligne de la source, un clic. |
...
- **Autonomie** : 🟡.
- **Multi-scène, étape 1 — FAIT ET PROUVÉ (2026-07-24)** : créer/lister/basculer entre
...
| 2026-08-04→06 | Takumi (session dév) | 🧱 **9 briques + persistance — 35 commits (`5ad9618`→`605293a`).** Multi-scènes étape 2 **prouvée** (point rouge levé) · étape 3 (panneau dédié) partielle · B7 souris **prouvée** · B6 tranches 1-3 (mixeur, anti-bruit, volumes casque/public + fenêtre de réglages) · **B-sources** brique neuve **prouvée** (jeu/fenêtre/écran/image/vidéo, liseré, aimantation, recherche) · **B-persist** scènes+sources **prouvées**, caméra+audio **non prouvées**. Arbitrage de Jay : *« pas suffisamment avancé pour que je puisse dire que je vais l'utiliser »* → le ducking passe après l'utilisabilité, d'où B-sources. Tests : 134 JS + 190 Rust verts, 0 lint. **Fait marquant : 12 défauts sur 12 trouvés à l'écran, 0 par les tests** — ils tournent hors du navigateur de l'app. Détail + 4 leçons : `docs/Sessions/Session-2026-08-06-001.md`. |
